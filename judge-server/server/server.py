...
def heartbeat_worker():
    """Background thread to send periodic heartbeats to the backend"""
    ...
    try:
        ...
        while True:
            ...
            # Send heartbeat every 5 seconds (backend checks every 6 seconds)
            time.sleep(5)
    except Exception as e:
        logger.exception(f"Failed to start heartbeat worker: {e}")


# Start heartbeat thread when the app is loaded
