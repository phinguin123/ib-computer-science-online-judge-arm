...
function ajax (url, method, options) {
  ...
  return new Promise((resolve, reject) => {
    axios({
      ...
      // API returns normally (status=20x), check if there's an error by checking the error field
      if (res.data.error !== null) {
        Vue.prototype.$error(res.data.data)
        reject(res)
        // If backend returns login request, session has expired, should logout current user
